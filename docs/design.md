...
### 5.1b `shots` 改成「版式引擎 + 风格」 —— **2026-09-21**

参照物是 Rebel Girls 的商店页：前两张一台手机跨缝拆开，底部的地形线贯穿整套五张（逐像素对过接缝）。在 od-mobile 上原型迭代了四轮，Ken 定下的规矩写死在引擎里，风格改不了：

- **整套只有一种手机尺寸**；横屏就是同一台手机横过来，放不下就出血截断（`anchor` start / end），不许为了塞进去而缩小——缩小让每张比例都不同，一排看下来像一堆。
- **只有 hero 能倾斜**；乱角度是「乱」的另一半。
- **尽量一张一台**；两台是例外，且两台的帧不能相邻（TRSC、LEAD 连着两张横竖搭配就乱）。
- 编号按实际张数（hero 占两张）；logo 可以从真实截屏里裁。

实现：content 里没有坐标（`frames` + `screen`），`layout.ts` 按规则算位置（纯函数），`paint.ts` 把整套排成一个 N·W 宽的舞台、每张平移 -i·W 截一次——接缝构造上对齐，不需要图像库切图；属于某张的元素裁在本张内（原型第一轮出血的手机漏到隔壁去过）。外框 CSS 画，不画灵动岛（simctl 截屏里自带）。风格是注册表里的一项（`plain` 保留旧样子、`stage`、`color`），项目风格用 `extendStyle`；OverDrive 的和弦级数小节格这种只属于一个 App 的装饰放项目风格，不进内置。

**旧写法（`Shot[]` / `cards` / `template`）直接删除，不留兼容**（Ken 的决定）；od-mobile 依赖的是 npm 上的 `^0.2.2`，不会被动升级，迁移时跟着改 content。
