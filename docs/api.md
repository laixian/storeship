...
| Screenshots | `defineShots`, `STYLES`, `extendStyle`, `styleKit`, `resolveTheme`, `expand`, `layout`, `paintSlot`, `deviceHtml`, `checkShots`, `stageFor`, `renderSet`, `shoot`, `sheet`, `pngSize`, `findChrome`, `loadShots`, `uploadShots`, `DEVICES`, `PREVIEW_TYPE`, `mergeDevices`, and the `Frame` / `ShotsContent` / `Style` / `Device` / `Placed` types |
...
The pure functions (`parseListing`, `diffFields`, `xfadeChain`, `steadyRuns`, `checkShots` with an injected fs, `layout`, `paintSlot`, `signJwt`, `stageOf`) have no I/O and are the ones with tests; the client accepts a `fetch` for testing against a fake.
