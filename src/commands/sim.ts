...
export const simCommand: Command = {
  ...
  sub: [
    ...
    { name: 'statusbar', summary: 'override the status bar to 9:41 / full battery / full signal', usage: 'sim statusbar [--time 9:41]', flags: { time: 'clock text; default 9:41' }, run: async (ctx) => { const s = await sim(ctx); s.statusBar(ctx.args.str('time')); ctx.out.emit({ statusBar: 'overridden' }); ctx.out.log('status bar overridden') } },
