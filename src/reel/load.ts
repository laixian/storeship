...
export async function loadReel(cfg: Config): Promise<{ content: ReelContent; template: ReelTemplate }> {
  ...
  if (!f || !existsSync(f)) throw new StoreshipError(`reel content not found: ${f ?? '(unset)'}`, 'set reel.content in storeship.config.json (a .ts/.js/.json exporting { canvas, band, crop?, copy })')
  const m = await mod(f)
  const content: ReelContent = m.default ?? m.content ?? m
  for (const k of ['canvas', 'band'] as const) if (!content[k]) throw new StoreshipError(`${f}: missing "${k}"`)
  let template: ReelTemplate = m.template ?? defaultReelTemplate
  if (cfg.reel.template) {
    if (!existsSync(cfg.reel.template)) throw new StoreshipError(`reel template not found: ${cfg.reel.template}`)
    const t = await mod(cfg.reel.template)
    template = t.default ?? t.template ?? t
    if (typeof template.render !== 'function') throw new StoreshipError(`${cfg.reel.template} must export a template with render(ctx)`)
  }
